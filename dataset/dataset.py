...
def _train_loader(c: SourceCodeDataConfigs):
    if os.path.exists("train_dataset.pkl"):
        dataloader = pickle.load(open('train_dataset.pkl', 'rb'))
    else:
        dataloader = DataLoader(TokenDataset(data=c.text_to_i(c.dataset.train, is_silent=False),
                                ...
                                drop_last=True)
        pickle.dump(dataloader, open('train_dataset.pkl', 'wb'))
    return dataloader
...
def _valid_loader(c: SourceCodeDataConfigs):
    if os.path.exists("valid_dataset.pkl"):
        dataloader = pickle.load(open('valid_dataset.pkl', 'rb'))
    else:
        dataloader = DataLoader(TokenDataset(data=c.text_to_i(c.dataset.valid, is_silent=False),
                                             batch_size=c.batch_size,
                                             seq_len=c.seq_len,
                                             drop_last=True),
                                batch_size=c.batch_size,
                                ...
                                shuffle=c.is_shuffle,
                                drop_last=True)
        pickle.dump(dataloader, open('valid_dataset.pkl', 'wb'))
    return dataloader
